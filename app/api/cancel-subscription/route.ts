...
export async function POST() {
  ...
  try {
    const res = await fetch(
      `${CREEM_API_BASE}/v1/subscriptions/${sub.creem_subscription_id}/cancel`,
      {
        method: "POST",
        headers: {
    ...
    if (!res.ok) {
      ...
      console.error("Creem cancel error:", res.status, text);
      return NextResponse.json(
    ...
    if (result.error) {
      console.error("[cancel] update error:", result.error.message);
      return NextResponse.json(
        { error: "Local update failed: " + result.error.message },
        { status: 500 },
      );
    }
